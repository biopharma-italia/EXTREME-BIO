 ...
 * 
 * Pipeline: requestId → cors → rateLimit → auth → handler → audit → errorHandle
 */
...
const RATE_LIMITS: Record<string, { max: number; windowMs: number }> = {
  ...
  '/api/auth/verify-2fa': { max: 5, windowMs: 60_000 },
  'default_authenticated': { max: 60, windowMs: 60_000 },
  'file_upload': { max: 10, windowMs: 60_000 },
  'file_download': { max: 30, windowMs: 60_000 },
};
...
// ── RBAC Helper (exported for route handlers) ───────────────────────────────
