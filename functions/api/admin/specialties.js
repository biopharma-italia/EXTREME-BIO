...
/**
 * Bio-Clinic Admin API — Specialties CRUD
 * GET/POST/PATCH/DELETE /api/admin/specialties
 * @version 1.0.0
 */

const { onRequestGet, onRequestPost, onRequestPatch, onRequestDelete, onRequestOptions } =
  createCrudHandlers('specialties', {
    selectFields: 'id,name,name_extended,slug,parent_id,icon,color_primary,description_short,show_in_menu,show_in_homepage,featured,menu_order,status,created_at,updated_at',
    searchFields: ['name', 'name_extended', 'description_short'],
    defaultSort: 'menu_order',
    defaultOrder: 'asc',
    requiredFields: ['id', 'name', 'slug'],
    validators: {
      slug: (v) => /^[a-z0-9-]+$/.test(v) ? null : 'Slug must be lowercase alphanumeric with dashes',
    },
  });
