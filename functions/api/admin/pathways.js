...
/**
 * Bio-Clinic Admin API — Pathways CRUD
 * GET/POST/PATCH/DELETE /api/admin/pathways
 * @version 1.0.0
 */

const { onRequestGet, onRequestPost, onRequestPatch, onRequestDelete, onRequestOptions } =
  createCrudHandlers('pathways', {
    selectFields: 'id,name,name_extended,name_short,slug,type,description_short,clinical_benefit,target_audience,duration,expected_outcome,is_flagship,priority_score,price_total,page_url,status,created_at,updated_at',
    searchFields: ['name', 'name_extended', 'description_short', 'clinical_benefit'],
    defaultSort: 'priority_score',
    defaultOrder: 'desc',
    requiredFields: ['id', 'name', 'slug'],
    validators: {
      type: (v) => ['clinical_pathway', 'program', 'checkup_pathway'].includes(v) ? null : 'Invalid pathway type',
    },
  });
