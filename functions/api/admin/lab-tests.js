...
/**
 * Bio-Clinic Admin API — Lab Tests CRUD
 * GET/POST/PATCH/DELETE /api/admin/lab-tests
 * @version 1.0.0
 */

const { onRequestGet, onRequestPost, onRequestPatch, onRequestDelete, onRequestOptions } =
  createCrudHandlers('lab_tests', {
    selectFields: 'id,name,name_extended,code,category,subcategory,sample_type,fasting_required,turnaround_time,description_short,price,price_urgent,symptoms,status,created_at,updated_at',
    searchFields: ['name', 'name_extended', 'code', 'category'],
    defaultSort: 'name',
    defaultOrder: 'asc',
    requiredFields: ['id', 'name', 'category'],
    validators: {
      price: (v) => (v === null || v === undefined || (typeof v === 'number' && v >= 0)) ? null : 'Price must be a positive number',
    },
  });
