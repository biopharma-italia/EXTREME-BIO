...
/**
 * Bio-Clinic Admin API — Packs CRUD
 * GET/POST/PATCH/DELETE /api/admin/packs
 * @version 1.0.0
 */

const { onRequestGet, onRequestPost, onRequestPatch, onRequestDelete, onRequestOptions } =
  createCrudHandlers('packs', {
    selectFields: 'id,name,name_extended,slug,type,category,description_short,tagline,target_audience,exams_included,exams_count,price_total,savings_pct,status,created_at,updated_at',
    searchFields: ['name', 'name_extended', 'description_short'],
    defaultSort: 'name',
    defaultOrder: 'asc',
    requiredFields: ['id', 'name', 'slug'],
    validators: {
      type: (v) => ['checkup', 'screening', 'pack_lab', 'pack_specialist'].includes(v) ? null : 'Invalid pack type',
    },
  });
