...
/**
 * Bio-Clinic Admin API — Physicians CRUD
 * GET/POST/PATCH/DELETE /api/admin/physicians
 * @version 1.0.0
 */

const { onRequestGet, onRequestPost, onRequestPatch, onRequestDelete, onRequestOptions } =
  createCrudHandlers('physicians', {
    selectFields: 'id,title,first_name,last_name,name,display_name,specialty_id,specialties_secondary,job_title,role_badges,is_director,bio_short,photo_url,booking_enabled,miodottore_url,miodottore_id,availability,show_in_equipe,show_in_specialty,featured_on_homepage,priority_order,status,created_at,updated_at',
    searchFields: ['first_name', 'last_name', 'job_title', 'bio_short'],
    defaultSort: 'priority_order',
    defaultOrder: 'asc',
    requiredFields: ['id', 'title', 'first_name', 'last_name', 'specialty_id'],
    validators: {
      title: (v) => ['Prof.', 'Dott.', 'Dott.ssa'].includes(v) ? null : 'Title must be Prof., Dott., or Dott.ssa',
    },
  });
