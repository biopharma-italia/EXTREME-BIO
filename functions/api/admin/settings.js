...
/**
 * Bio-Clinic Admin API — Settings (M10)
 * GET/PUT/PATCH /api/admin/settings
 * ==========================================
 * Key-value configuration store for site and admin settings.
 *
 * GET:   Returns all settings (grouped by category)
 * PUT:   Bulk update all settings { settings: { key: value, ... } }
 * PATCH: Update a single setting { key, value }
 *
 * @version 2.0.0
 * @date 2026-02-28
 */
...
export async function onRequestGet(context) {
  return withAdminAuth(context, async (ctx, user) => {
    ...
    try {
      ...
      const { data } = await db.query('settings', {
      ...
      return jsonResponse({ settings: grouped }, 200, request);
    } catch (err) {
      console.error('[Settings GET Error]', err);
      return errorResponse('Failed to load settings: ' + err.message, 500, request);
    }
  });
}
...
export async function onRequestPut(context) {
  return withAdminAuth(context, async (ctx, user) => {
    ...
    if (!['super_admin', 'admin'].includes(user.role)) {
      return errorResponse('Insufficient permissions to modify settings', 403, request);
    }
    ...
    try {
      ...
      return errorResponse('Failed to save settings: ' + err.message, 500, request);
    }
  });
}
...
export async function onRequestPatch(context) {
  return withAdminAuth(context, async (ctx, user) => {
    ...
    if (user.role !== 'super_admin') {
      return errorResponse('Only super_admin can modify settings', 403, request);
    }
    ...
    try {
      ...
      if (!key) {
        return errorResponse('Missing required field: key', 400, request);
      }

      const result = await db.update('settings', key, {
      ...
      return errorResponse('Failed to update setting: ' + err.message, 500, request);
    }
  });
}
