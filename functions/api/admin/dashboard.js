...
/**
 * Bio-Clinic Admin API — Dashboard Stats
 * GET /api/admin/dashboard
 * ==========================================
 * Returns aggregated statistics for the admin dashboard.
 *
 * Response: {
 *   stats: { active_specialties, active_physicians, ... },
 *   recent_contacts: [...],
 *   upcoming_bookings: [...]
 * }
 *
...
export async function onRequestGet(context) {
  return withAdminAuth(context, async (ctx, user) => {
    ...
    try {
      ...
      return jsonResponse({
        stats: {
          active_specialties: specialtiesRes.count || specialtiesRes.data?.length || 0,
          active_physicians: physiciansRes.count || physiciansRes.data?.length || 0,
          active_procedures: proceduresRes.count || proceduresRes.data?.length || 0,
      ...
      return errorResponse('Failed to load dashboard: ' + err.message, 500, request);
    }
  });
}
