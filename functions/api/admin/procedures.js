...
/**
 * Bio-Clinic Admin API — Procedures CRUD
 * GET/POST/PATCH/DELETE /api/admin/procedures
 * @version 1.0.0
 */

const { onRequestGet, onRequestPost, onRequestPatch, onRequestDelete, onRequestOptions } =
  createCrudHandlers('procedures', {
    selectFields: 'id,name,name_extended,slug,specialty_id,category,page_type,description_short,duration_minutes,preparation_required,fasting_required,show_price,price,price_note,booking_priority,page_url,status,created_at,updated_at',
    searchFields: ['name', 'name_extended', 'description_short'],
    defaultSort: 'name',
    defaultOrder: 'asc',
    requiredFields: ['id', 'name', 'slug', 'specialty_id', 'category'],
    validators: {
      category: (v) => ['visita', 'diagnostica', 'esame', 'trattamento', 'intervento', 'terapia'].includes(v) ? null : 'Invalid category',
      page_type: (v) => ['A', 'B', 'C', 'D'].includes(v) ? null : 'Page type must be A, B, C, or D',
    },
  });

export { onRequestGet, onRequestPost, onRequestPatch, onRequestDelete, onRequestOptions };
